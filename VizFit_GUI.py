...
class ExercisesER():
    def drawSquatsPoints(self, pointsList, counters, durations, img):
        try:
            ...
            backStrightL = abs((y23-y11)/(x23-x11)) #Shoulder to hip slope L
            backStrightR = abs((y24-y12)/(x24-x12)) #Shoulder to hip slope R
            
            footToKneeL = abs((y27-y25)/(x27-x25)) #Foot to knee slope L
            footToKneeR = abs((y28-y26)/(x28-x26)) #Foot to knee slope R
            
            shoulderWidth = abs(x12-x11)
            footWidth = abs(x28-x27)
            
            if shoulderWidth-5 > footWidth or shoulderWidth+5 < footWidth:
                shouldersToFootWidth = 1
            
            hipsToKneesHeightL = y25-y23
            hipsToKneesHeightR = y26-y24
            
            if hipsToKneesHeightL < 0 or hipsToKneesHeightR < 0:
                hipsBelowKnees = 1
            ...
            if averagePercentLF == 100:
                if durations[0] == 0:
                    if hipsBelowKnees == 0 and shouldersToFootWidth == 0 and backStrightL > 1 and backStrightL < 3.5 and backStrightR > 0 and backStrightR < 2 and footToKneeL > 0 and footToKneeL < 2 and footToKneeR > 0 and footToKneeR < 2:
                        counters[0] += 0.5
                        durations[0] = 1
                    else:
                        goodSU = 0
            ...
            if averagePercentLF <= 50:
                if durations[0] == 1:
                    if goodSU != 1 and hipsBelowKnees == 0 and shouldersToFootWidth == 0 and backStrightL > 1 and backStrightL < 3.5 and backStrightR > 0 and backStrightR < 2 and footToKneeL > 0 and footToKneeL < 2 and footToKneeR > 0 and footToKneeR < 2:
                        counters[0] += 0.5
                        durations[0] = 0
                    else:
                        goodSU = 0
    ...
    def isInFrame(self, pointsList, durations, img):
        try:
...
def runExerciseRecogination():
    ...
    while True:
        ...
        rectangleColor =  (67, 67, 67)#(0,250,0)
        textColor = (255, 255, 255)
        textSize = 1.5

        if exercise.isInFrame(pointList, durations, img) == True:
            ...
            if counters[3] == 0:
                ...
                cv2.putText(img, (str("2) Back stright")), (740, 95), cv2.FONT_HERSHEY_PLAIN, textSize, textColor, 1)
                cv2.putText(img, (str("3) Knees in line with your toes")), (740, 125), cv2.FONT_HERSHEY_PLAIN, textSize, textColor, 1)
                cv2.putText(img, (str("4) Heels on the floor")), (740, 155), cv2.FONT_HERSHEY_PLAIN, textSize, textColor, 1)
            ...
            # Percent Of 'Good' Exercises #
            if counters[0] != 0:
                squatsGoodProb = squatsGoodCount/counters[0]
            if counters[1] != 0:
                pushUpsGoodProb = pushUpsGoodCount/counters[1]
            if counters[2] != 0:
                sitUpsGoodProb = sitUpsGoodCount/counters[2]
                
        else:
            cv2.rectangle(img, (655, 720), (0, 595), (0,0,250), cv2.FILLED)
            #cv2.rectangle(img, (650, 720), (0, 600), (60,20,220), cv2.FILLED)
    ...
    def verifyLogin(self):
        ...
        if (username == "" and password == ""):
            ...
            try:
                ...
                returnedUserId = -1
                hasError = True
            hasError = False ##################################################REMOVE!!! FOR TESTING ONLY!!!
            if hasError == False:
                if True:#returnedUsername != "None" and returnedPassword != "None":
...
class RegisterWindow(QDialog):
    ...
    def verifyRegisteration(self):
        ...
        if (username == "" or password == "" or confirmPassword == ""):
            ...
            global registrationSuccessful
            registrationSuccessful = True#False
            hasError = False
            ...
            try:
                ...
                hasError = True
            hasError = False ##################################################REMOVE!!! FOR TESTING ONLY!!!
            response = "1" ####################################################REMOVE!!! FOR TESTING ONLY!!!
            if hasError == False:
                if response != "-999":
    ...
    def __init__(self):
        ...
        if numSquats == 0 and numPushups == 0 and numSitups == 0:
            ...
            try:
                ...
                hasError = True
            hasError = False ##################################################REMOVE!!! FOR TESTING ONLY!!!
            response = 200 ####################################################REMOVE!!! FOR TESTING ONLY!!!
            if hasError == False:
                if response == 200:
